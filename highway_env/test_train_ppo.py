...
attention_network_kwargs = dict(
    in_size=5*15,
    embedding_layer_kwargs={"in_size": 7, "layer_sizes": [128, 128], "reshape": False},
...
if __name__ == "__main__":
    ...
    if train:
        ...
        # Save the agent
        model.save("ppo-highway4")

    #retrain the training
    if retrain:
        n_cpu = 4
        env = make_vec_env(make_configure_env, n_envs=n_cpu, seed=0, vec_env_cls=SubprocVecEnv, env_kwargs=env_kwargs)
        model = PPO.load("ppo-highway4",env,
                    learning_rate=1e-3)
        #env = model.get_env()
        model.learn(total_timesteps=400*1000)
